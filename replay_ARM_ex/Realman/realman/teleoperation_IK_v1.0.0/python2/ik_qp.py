...
class QPIK():
    ...
    def set_joint_limit_max(self, angle, unit = 'rad'):
        if len(angle)!= self.robot.dof:
            raise "[ERROR] joint_limit_max size should be self.robot.dof."
    ...
    def set_joint_limit_min(self,angle, unit = 'rad'):
        if len(angle)!= self.robot.dof:
            raise "[ERROR] joint_limit_min size should be self.robot.dof."
        for i in range(self.robot.dof):
            if unit == 'deg':
    ...
    def set_error_weight(self, weight):
        if len(weight)!= 6:
            raise "[ERROR] weight size should be 6."
        for i in range(6):
            if weight[i] > 1.0:
                weight[i] = 1.0
            elif weight[i] < 0.0:
    ...
    def set_joint_velocity_limit(self, dq_max, unit='rad'):
        if len(dq_max)!= self.robot.dof:
            raise "[ERROR] dq_max size should be self.robot.dof."
        for i in range(self.robot.dof):
            if unit == 'deg':
    ...
    def set_dq_max_weight(self, weight):
        if len(weight)!= self.robot.dof:
            raise "[ERROR] weight size should be self.robot.dof."
        for i in range(self.robot.dof):
            if weight[i] > 1.0:
                weight[i] = 1.0
            elif weight[i] < 0.0:
    ...
    def sovler(self, q_ref, Td, max_iter = 150):
        ...
        self.c = -2*np.dot(np.dot(np.dot(self.Jaco.transpose() , self.W.transpose()) , self.W), self.DX)
        self.P =  2* np.dot(np.dot(np.dot(self.Jaco.transpose() , self.W.transpose()) , self.W) , self.Jaco)
